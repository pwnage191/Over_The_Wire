...
#### My path: 
  ...
  - It contained the file name dictionary.txt, i opened that file, it had the list of words.
  - In the code, I saw the line "passthru("grep -i $key dictionary.txt");".
  - I used Google to learn that passthru executes system commands, similar to exec().
  - So, it is a command line injection.
  - Case 1:
      When I entered the "hello" into the input, the program executes "grep -i hello dictionary.txt".
      This searches for the word "hello" (case‑insensitive) inside dictionary.txt and prints matching lines.
  - Case 2:
      When I entered the "cat /etc/natas_webpass/natas10", It only takes cat to search inside the dictionary.txt, 
      and showed the matching letter.because the input is appended directly after grep, not executed as a separate command.
  - Case 3:
      So, When I entered hello;cat /etc/natas_webpass/natas10, the php code executes two separate commands,
